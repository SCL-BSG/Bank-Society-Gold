...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSocietyGAmount"))
        settings.setValue("nAnonymizeSocietyGAmount", 1000);
    nAnonymizeSocietyGAmount = settings.value("nAnonymizeSocietyGAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSocietyGAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSocietyGAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSocietyGAmount:
            return QVariant(nAnonymizeSocietyGAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSocietyGAmount:
            nAnonymizeSocietyGAmount = value.toInt();
            settings.setValue("nAnonymizeSocietyGAmount", nAnonymizeSocietyGAmount);
            emit AnonymizeSocietyGAmountChanged(nAnonymizeSocietyGAmount);
            break;
        default:
            break;
        }
