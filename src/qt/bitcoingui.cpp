...
{
    resize(900, 520);
    setWindowTitle(tr("Bank Society Gold") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("BSG");
    setStyleSheet("#BSG {background-color: qradialgradient(cx: 0, cy: 1, fx: 0, fy: 0, radius: 1, stop: 0 #ffffff, stop: 1 #e0e0e0);}");
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Bank SocietyG address"));
    ...
    aboutAction = new QAction(tr("&About Bank SocietyG"), this);
    aboutAction->setToolTip(tr("Show information about Bank Society Gold"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for Bank Society Gold"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Bank SocietyG client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Bank SocietyG client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Bank SocietyG network", "", count));
}
...
{
    QString strTitle = tr("Bank Society Gold") + " - ";
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Bank SocietyG address or malformed URI parameters."));
    }
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    if ( nLastCoinStakeSearchInterval && nWeight)
    {
        ...
        nStakeWeight /= COIN;
        nNetworkWeight ; // /= COIN; rgp edited
