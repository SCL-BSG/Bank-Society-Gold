...
struct CNodeState {
    std::string name;
    // RGP these tow lines can be removed later
    // List of asynchronously-determined block rejections to notify this peer about.
    // std::vector<CBlockReject> rejects;
    list<QueuedBlock> vBlocksInFlight;
    int nBlocksInFlight, nMisbehavior;
    bool fShouldBan;
...
bool AddOrphanTx(const CTransaction& tx)
{
    ...
    if (mapOrphanTransactions.count(hash)){
        LogPrintf("*** RGP AddOrphanTx FAIL check on HASH \n");
        return false;
    }
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    {
        LogPrintf("*** RGP Max size is %d orphan transaction size is %d \n", nMaxOrphans, mapOrphanTransactions.size()  );
...
bool AreInputsStandard(const CTransaction& tx, const MapPrevTx& mapInputs)
{
     ...
     LogPrintf("*** RGP AreInputsStandard \n");
...
unsigned int GetP2SHSigOpCount(const CTransaction& tx, const MapPrevTx& inputs)
{
    ...
    //LogPrintf("*** RGP GetP2SHSigOpCount \n");
...
{
     LogPrintf("*** RGP WantedByOrphan Optimised \n");
...
void static PruneOrphanBlocks()
{
   ...
   LogPrintf("*** RGP PruneOrphanBlocks - OPtimised \n");
    ...
    /* RGP, WTF is this coding, hacking inc? */
    LogPrintf("*** RGP PruneOrphanBlocks - OPTIMISE THIS SHIT CODE \n");
    while (pos--) it++;
...
/* --------------------------------------------------------------------------- 
   -- RGP, Bank SocietyG Coin Information				    --
   ---------------------------------------------------------------------------
...
double GetDynamicBlockReward3(int nHeight)
{
        ...
        */
        ...
        if (nHeight > 0)
        {
            nSubsidyMax = 200000; /* RGP Block 1 to 100 - Premine */
        }
        /* ------ Initial Mining Phase: Block #101 Up to 50000 ------ */
        if (nHeight > 100)
        {
            nSubsidyMax = 5; /* RGP Block 101 to 50000 - PoW Start */
        }

        /* RGP, removed, this is not defined in any previous coin specification */
        ...
        /* ------ Regular Mining Phase: Block #50001 Up to 250000 ------ */
        if (nHeight > 50001)
        {
            nSubsidyMax = 25;
        }
        /* ------ Regular Mining Phase: Block #250001 Up to 500000 ------ */
        if (nHeight > 250001)
        {
            nSubsidyMax = 35;
        }
        /* ------ Regular Mining Phase: Block #500001 PoW End all rewards Mining stops ------ */
        if (nHeight > 750001)
        {
            nSubsidyMax = 0;
        }
...
/* ---------------------------------------------------------------------------
   -- RGP, Bank SocietyG Coin Proof of Work Rewards                          --
   ---------------------------------------------------------------------------
   -- Premine Phase		: Block 1 to 100        Reward = 200,000    --
   -- PoW Phase Start		: Block 101 to 50000 	Reward = 5          --
   -- PoW Phase  		: Block 50001 to 250000	Reward = 25	    --
...
/* ---------------------------------------------------------------------------
   -- RGP, Bank SocietyG Coin Proof of STake Rewards                         --
   ---------------------------------------------------------------------------
   -- Premine Phase	: Block 1 to 100                                    --
   -- PoS Phase Start	: Block 101 to 50000            Reward 50%          --
   -- PoS Phase  	: Block 50001 to 250000		Reward 250%         --
   -- PoS Phase		: Block 250001 to 500000	Reward 360%         --
   -- PoS Phase		: Block 500000			Reward 500%         --
   --------------------------------------------------------------------------- */
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    {
        nSubsidy = nSubsidy * 0.05;  // RGP  50% 500% was 0.5
    }
    /* ------ Initial Mining Phase: Block #50001 Up to 250000 ------ */
    ...
    {
        nSubsidy = nSubsidy * 0.36;  // 360% RGP 0.125; 125%
    }

    /* ------ Regular Mining Phase: Block #500001+ ------ */
    if (pindexPrev->nHeight > 500000)
    {
        nSubsidy = nSubsidyBase * 0.5; // RGP 500% 0.25; 250%
    }
...
/* RGP, changed to 60 seconds Block time
...
{
    LogPrintf("*** RGP InvalidChainFound \n");
...
void CBlock::RebuildAddressIndex(CTxDB& txdb)
{
     ...
     LogPrintf("*** RGP RebuildAddressIndex \n");
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    LogPrintf("*** RGP T E S T code SetBestChain block hash %s Height %l \n", hash.ToString(), pindexBest->nHeight );
...
extern bool stakingAllowed;
    ...
    if (mi == mapBlockIndex.end()){
        ...
        //if ( !stakingAllowed ){
            /* RGP : Special case during block synch where previous block is not
                     located anywhere. This needs to be fixed later by requesting
                     the missing block.                                            */
            //LogPrintf("*** RGP AcceptBlock fail");
       // }else
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock, bool importMode)
{
/* RGP Added nMacConnections here to reduce the number of connections, where all connections
   are sending the same data. Concept is to reduce to 3 to limit the number of conections
   and orphaned blocks to be processed.
   nMaxConnections is then recorvered when zero Orphaned blocks are then determined           */
extern int nMaxConnections ;
extern bool stakingAllowed;
uint256 hash, previous_hash;


    LogPrintf("*** RGP ProcessBlock Start \n");


    /* RGP, If IMPORTING then do not allow other routines to access */

    if ( !importMode ){
        while( stakingAllowed == false ){
            LogPrintf("*** RGP ProcessBlock - NO processing IMPORT IN PROGRESS\n");
            MilliSleep(10000);
            return error("ProcessBlock() : no staking allowed at this time \n" );
        }
    }
    ...
    if (mapBlockIndex.count(hash)){
        LogPrintf("*** RGP ProcessBlock Start, ALREADy have block hash %s \n", hash.ToString());
        return error("ProcessBlock() : already have block %d %s", mapBlockIndex[hash]->nHeight, hash.ToString());
    }

    /* Suspect that blocks sent from top to bottom, this means that 751 Orphan blocks is usless
       -- if we are bootstrapping */
    LogPrintf("*** RGP T E S T code Block Hash %s prev hash %s \n", hash.ToString(), pblock->hashPrevBlock.ToString() );
    if ( mapBlockIndex.count( pblock->hashPrevBlock ) ){
        LogPrintf("*** RGP T E S T code Block Hash %s prev hash %s PREV FOUND\n", hash.ToString(), pblock->hashPrevBlock.ToString() );
    }

    if (mapOrphanBlocks.count(hash)){
        LogPrintf("*** RGP ProcessBlock Start, ALREADy have block ORPHAN hash %s \n", hash.ToString());
    ...
    if (!fReindex && !fImporting && pblock->IsProofOfStake() && setStakeSeen.count(pblock->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash)){
        LogPrintf("*** RGP ProcessBlock Start, duplicate proof-of-stake %s \n", hash.ToString());
    ...
    LogPrintf("*** RGP ProcessBlock Start, Prev Block %s bestchain hash %s \n", pblock->hashPrevBlock.ToString(), hashBestChain.ToString()   );
    ...
    LogPrintf("*** RGP ProcessBlock Debug 001A \n");
    ...
    if (!IsCanonicalBlockSignature(pblock)) {
        LogPrintf("*** RGP ProcessBlock Debug 001B \n");
        return error("ProcessBlock(): bad block signature encoding");
    }

    // Preliminary checks
    if (!pblock->CheckBlock()){
        LogPrintf("*** RGP ProcessBlock Debug 001C \n");
...
LogPrintf("ProcessBlock: ORPHAN BLOCK %lu, prev=%s\n", (unsigned long)mapOrphanBlocks.size(), pblock->hashPrevBlock.ToString());
        ...
        /* --------------------------------------------------------------------
           -- RGP, reduce nMaxConnections to 3 on detection of Orphan blocks --
           --      recover back to 125 after orthan blocks are no longer     --
           --      detected.                                                 --
           -------------------------------------------------------------------- */

         nMaxConnections = 125;       /* this affects nett.cpp processing of incoming connections */
         LogPrintf("*** RGP nMaxConnections set to 3 \n");
        ...
        if (pfrom) {
            ...
            if (pblock->IsProofOfStake())
            {
                ...
                LogPrintf("*** RGP ProcessBlock Debug PoS 001E \n");
                // Limited duplicity on stake: prevents block flood attack
                // Duplicate stake allowed only when there is orphan child block
                if (setStakeSeenOrphan.count(pblock->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash)){
                    LogPrintf("*** RGP ProcessBlock Debug duplicate orphane seen  001F \n");
            ...
            LogPrintf("*** RGP ProcessBlock Debug PoS 001H \n");
            ...
            LogPrintf("*** RGP ProcessBlock Debug PoS 001J \n");
    ...
    LogPrintf("*** RGP Before AcceptBlock 001Ja \n");

    // Store to disk
    if (!pblock->AcceptBlock()){
        LogPrintf("*** RGP AcceptBlock FAILED *** \n");
        return error("ProcessBlock() : AcceptBlock FAILED");
    }


    LogPrintf("*** RGP ProcessBlock Debug RECURSIVE CRAP 001K \n");
...
LogPrintf("*** RGP ProcessBlock time start for MasternodePayments process block \n");
...
LogPrintf("*** Masternodepayments Processblock took %15dms\n", GetTimeMillis() - nStart);
    ...
    /* -----------------------------------------------------------------------
       -- RGP, returm nMaxConnections to 125 on detection of accepted block --
       ----------------------------------------------------------------------- */

     nMaxConnections = 125;       /* this affects nett.cpp processing of incoming connections */
     LogPrintf("*** RGP nMaxConnections set to 125 \n");
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    LogPrintf("*** RGP LoadExternalBlockFile start \n");

    nLoaded = 0;        /* RGP, Blocks load count */
    importState = true;

    {
        try {
            ...
            {
                LogPrintf("*** RGP Importing block %l \n", nLoaded );
                ...
                do {
                    ...
                    if (nRead <= 8)
                    {
                        LogPrintf("*** RGP LoadExternalBlockFile start nRead %d fail \n", nRead);
                        nPos = (unsigned int)-1;
                        break;
                    }
                ...
                if (nSize > 0 && nSize <= MAX_BLOCK_SIZE)
                {
                    ...
                    blkdat >> block;
                    LOCK(cs_main);
                    if (ProcessBlock( NULL,&block, importState ) )
                    {
                        nLoaded++;
                        nPos += 4 + nSize;
                    }
    ...
    LogPrintf("Loaded %d blocks from external file in %dms\n", nLoaded, GetTimeMillis() - nStart);
...
extern bool stakingAllowed;
    ...
    if (filesystem::exists(pathBootstrap)) {
        ...
        LogPrintf("*** RGP LOading Bootstrat.dat \n");
    ...
    LogPrintf("*** RGP ThreadImport ending, setting stakingAllowed to TRUE ");
...
bool static AlreadyHave(CTxDB& txdb, const CInv& inv)
{
    ...
    LogPrintf("*** RGP FAILED, NOTHING IDENTIFIED \n");
    return true;
}
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        if (pfrom->nSendSize >= SendBufferSize()){
            LogPrintf("*** RGP ProcessGetData, buffer too full \n");
            break;
        }
...
bool importState;
    ...
    //while( stakingAllowed == true ){
       // LogPrintf("*** RGP ProcessMessage - NO Processing during IMPORT ALLOWED\n");
       // MilliSleep(10000);
   // }
    ...
    LogPrintf("*** RGP ProcessMessage Start command is %s \n", strCommand );
    ...
    {
         LogPrintf("*** RGP version messages from %s version is %d \n", pfrom->addr.ToString() , pfrom->nVersion);
        ...
        if (pfrom->nVersion != 0)
        {
            ...
            LogPrintf("*** RGP Too many version messages %d \n", pfrom->nVersion );
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION)
        {
            ...
            LogPrintf("*** RGP version messages using wrong ptotocol from %s version is %d \n", pfrom->addr.ToString() , pfrom->nVersion);
        ...
        LogPrintf("*** RGP version messages valid ptotocol from %s version is %d \n", pfrom->addr.ToString() , pfrom->nVersion);
    ...
    else if (pfrom->nVersion == 0)
    {
        ...
        LogPrintf("*** RGP VERSION message is ZERO \n");
    ...
    {
        LogPrintf("*** RGP VERSION message is verack from %s version %d \n",pfrom->addr.ToString(), pfrom->nVersion );
        pfrom->SetRecvVersion(min(pfrom->nVersion, PROTOCOL_VERSION));
    }

    /* -----------------------------
       -- addr COMMAND Processing --
       ----------------------------- */

    else if (strCommand == "addr")
    {
        LogPrintf("*** RGP VERSION message is addr \n");
        ...
        if (pfrom->nVersion < CADDR_TIME_VERSION && addrman.size() > 1000){
            LogPrintf("*** RGP ProcessMess version check %d failed \n", pfrom->nVersion);
            return true;
        }

        if (vAddr.size() > 1000)
        {
            LogPrintf("*** RGP message is addr > 1000 Misbehaving \n");

            Misbehaving(pfrom->GetId(), 10);
            return error("message addr size() = %u", vAddr.size());
        }

        LogPrintf("*** RGP message is addr start processing \n");
        ...
        LogPrintf("*** RGP message is addr  before update DEBUG 001 now %l since %l \n", nNow, nSince );

        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            boost::this_thread::interruption_point();

            LogPrintf("*** RGP message is addr  before update DEBUG 001 since %l time %l \n", nSince, addr.nTime );

            if (addr.nTime <= 100000000 || addr.nTime > nNow + 10 * 60)
                addr.nTime = nNow - 5 * 24 * 60 * 60;

            LogPrintf("*** RGP message is addr  processing loop DEBUG 001 since %l time %l vaddr size %l \n", nSince, addr.nTime, vAddr.size() );

            pfrom->AddAddressKnown(addr);
            bool fReachable = IsReachable(addr);

            if (addr.IsRoutable() )
                LogPrintf("*** RGP message is addr  processing loop DEBUG 001a, is routable \n");
            else
                LogPrintf("*** RGP message is addr  processing loop DEBUG 001a, is NOT routable \n");

            if ( pfrom->fGetAddr )
                LogPrintf("*** RGP message is addr  processing loop DEBUG 001a, getaddr TRUE \n");
            else
                LogPrintf("*** RGP message is addr  processing loop DEBUG 001a, getaddr FALSE \n");


            // if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                LogPrintf("*** RGP message is addr relay to other modes \n");
                ...
                // Relay to a limited number of other nodes
                {
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        LogPrintf("*** RGP addr  processing loop DEBUG 003\n");
                ...
                LogPrintf("*** RGP addr  processing loop DEBUG 002 since %l time %l  not processed \n", nSince, addr.nTime );
            }
    ...
    /* -----------------------------------------------------------
       -- RGP, "inv" command is sending a list of network nodes --
       --      Remote is sending to here the Local node         --
       ----------------------------------------------------------- */
    ...
    else if (strCommand == "inv")
    {
        ...
        LogPrintf("*** RGP ProcessMessage inv command Debug 001 inventory size %d \n", vInv.size() );
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            MilliSleep(50); /* RGP OPtimise */
        }
        
        LOCK(cs_main);
        CTxDB txdb("r");

    LogPrintf("*** RGP ProcessMessage inv processing loop \n");
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            LogPrintf("*** RGP Remote node sent inventory: %s \n", inv.ToString() );


            if (!fAlreadyHave) {

                LogPrintf("*** RGP NEW inventory: %s \n", inv.ToString() );
                if (!fImporting){
                    LogPrintf("*** RGP IMPORTING \n");
                    pfrom->AskFor( inv, false );

                    MilliSleep(500);
                }

                /* RGP changed from ELSEIF */

                if ( fImporting ) {

                    LogPrintf("*** RGP NOT IMPORTING \n");

                    MilliSleep(150);

                    if (inv.type == MSG_BLOCK && mapOrphanBlocks.count(inv.hash)) {
                        LogPrintf("*** RGP Inventory MSG BLOCK store as orphan, we don't have \n" );
                        PushGetBlocks(pfrom, pindexBest, GetOrphanRoot(inv.hash));
                    } else if (nInv == nLastBlock) {
                               LogPrintf("*** RGP LAST BLOCK \n" );
    ...
    else if (strCommand == "getdata")
    {
        ...
        LogPrintf("*** RGP message is getdata \n");
        ...
        if (vInv.size() > MAX_INV_SZ)
        {
            LogPrintf("*** RGP message is getdata misbenhaving \n");
            Misbehaving(pfrom->GetId(), 20);
    ...
    else if (strCommand == "getblocks")
    {
        ...
        LogPrintf("*** RGP message is getblocks \n");

        CBlockLocator locator;
        uint256 hashStop;
        vRecv >> locator >> hashStop;

        LogPrintf("*** RGP getblocks \n");
        ...
        LogPrintf("*** RGP ProcessMessage getblocks processing loop \n");
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                LogPrintf("*** RGP getblocks hashStop at %d %s \n", pindex->nHeight, pindex->GetBlockHash().ToString() );
            ...
            if (--nLimit <= 0)
            {
                ...
                LogPrintf("*** RGP getblocks stopping at limit at %d %s \n", pindex->nHeight, pindex->GetBlockHash().ToString() );
    ...
    else if (strCommand == "tx"|| strCommand == "dstx")
    {
        ...
        LogPrintf("*** RGP  AddInventoryKnown \n");
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        int long curBlockHeight, curBlockTime;


LogPrintf("*** RGP block %s \n", hashBlock.ToString() );


        LogPrint("net", "received block %s\n", hashBlock.ToString());


        curBlockHeight = GetHeight();
        curBlockTime = pindexBest->GetBlockTime();


        LogPrintf("*** RGP BLOCK message, current height %d cur time %l new block time %l \n", curBlockHeight, curBlockTime, block.GetBlockTime() );

        CInv inv(MSG_BLOCK, hashBlock);
        pfrom->AddInventoryKnown(inv);

        LogPrint("*** RGP BLOCK MESSAGE, lastgetblocksbegin %l lastgetblockend %l \n", pnode->pindexLastGetBlocksBegin, pnode->hashLastGetBlocksEnd  );


        LOCK(cs_main);


        // Remember who we got this block from. RGP Test Code
        //mapBlockSource[inv.hash] = pfrom->GetId();
        //MarkBlockAsReceived(inv.hash, pfrom->GetId());

        if ( ProcessBlock(pfrom, &block, importState ) ){
            LogPrintf("*** RGP block is good, not seen the previous block yet %s \n", hashBlock.ToString() );
            mapAlreadyAskedFor.erase(inv);
        }

        if (block.nDoS){
            LogPrintf("*** RGP block.nDOS \n");
...
LogPrintf("*** RGP block end of rocessing %s \n", hashBlock.ToString() );
    ...
    else if ((strCommand == "getaddr") && (pfrom->fInbound))
    {
        ...
        BOOST_FOREACH(const CAddress &addr, vAddr)
            if(addr.nTime > nCutOff){
                LogPrintf("*** RGP vlock fingerprint cutoff OK %d \n", nCutOff );
                pfrom->PushAddress(addr);
            }
            else
                LogPrintf("*** RGP vlock fingerprint cutoff FAILED %d \n", nCutOff );
    }
    ...
    {
         LogPrintf("*** RGP PING command Received \n");
    ...
    {
         LogPrintf("*** RGP PONG command Received \n");
    ...
    LogPrintf("*** RGP ProcessMessage Exit \n");
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    if (!pfrom->vRecvGetData.empty()){
        LogPrintf("*** RGP before ProcessGetData from %s\n", pfrom->GetId());
        ProcessGetData(pfrom);
    }
    ...
    if (!pfrom->vRecvGetData.empty()) {
        ...
        LogPrintf("*** RGP vRecvGetData is not empty from %s\n", pfrom->GetId());
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        LogPrintf("*** RGP ProcessMessages iterate \n" );

        // Don't bother if send buffer is too full to respond anyway
        if (pfrom->nSendSize >= SendBufferSize()){
            LogPrintf("*** RGP ProcessMessages iterate Sendsize %d to big \n", pfrom->nSendSize );
            break;
        }

        LogPrintf("*** RGP ProcessMessages iterate Debug 001 \n" );

        // get next message
        CNetMessage& msg = *it;

        LogPrintf("*** RGP ProcessMessages iterate Debug 001a \n" );

           LogPrintf("ProcessMessages(message %u msgsz, %zu bytes, complete:%s)\n",
                     msg.hdr.nMessageSize, msg.vRecv.size(),
                     msg.complete() ? "Y" : "N");

        LogPrintf("*** RGP ProcessMessages iterate Debug 001b \n" );


        // end, if an incomplete message is found
        if (!msg.complete()){
            LogPrintf("*** RGP ProcessMessages iterate Message incomplete \n");
            break;
        }

        LogPrintf("*** RGP ProcessMessages iterate Debug 002 \n" );
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_START_SIZE) != 0) {
            ...
            fOk = false;
            LogPrintf("*** RGP ProcessMessages iterate Invalid message start \n");
            break;
        }

        LogPrintf("*** RGP ProcessMessages iterate Debug 003 \n" );
        ...
        LogPrintf("*** RGP ProcessMessages iterate Debug 004 \n" );
        ...
        LogPrintf("*** RGP ProcessMessages iterate Debug 005 \n" );
        ...
        try
        {
            ...
            LogPrintf("*** RGP ProcessMessages After call to ProcessMessage \n");

             MilliSleep(350); /* RGP OPtimise */
    ...
    LogPrintf("*** RGP ProcessMessages Exit \n");
...
LogPrintf("*** RGP SendMessages Debug 001 \n");

    TRY_LOCK(cs_main, lockMain);
    if (lockMain) {


        LogPrintf("*** RGP SendMessages Debug 002 \n");

        // Don't send anything until we get their version message
        if (pto->nVersion == 0)
            return true;

        LogPrintf("*** RGP SendMessages Debug 003 \n");
        ...
        LogPrintf("*** RGP SendMessages Debug 004 \n");
        ...
        if (pingSend) {
            uint64_t nonce = 0;
            while (nonce == 0) {
                LogPrintf("*** RGP SendMessages Debug 005 \n");
                GetRandBytes((unsigned char*)&nonce, sizeof(nonce));
                loopcheck++;
                if ( loopcheck > 100 ){
                    LogPrintf("*** RGP SendMessages Debug 006 \n");
                    break;
                }
            }
        ...
        LogPrintf("*** RGP SendMessages Debug 007 \n");
        ...
        LogPrintf("*** RGP SendMessages Debug 008 \n");
        ...
        LogPrintf("*** RGP SendMessages Debug 009 \n");
        ...
        {
            {
                ...
                BOOST_FOREACH(CNode* pnode, vNodes)
                {
                    ...
                    MilliSleep(20); /* RGP optimise */
                }
            }
            nLastRebroadcast = GetTime();
        }

        LogPrintf("*** RGP SendMessages Debug 0010 \n");
...
LogPrintf("*** RGP SendMessages Debug 0011 \n");
...
LogPrintf("*** RGP SendMessages Debug 0012 \n");
        ...
        //CNodeState *state = State( pto->GetId() );   // State(nodeid); RGP
        ...
        //    state->nBlocksInFlight < state->nLastBlockProcess - 2 * BLOCK_DOWNLOAD_TIMEOUT*1000000) {
        //    LogPrintf("Peer %s is stalling block download, disconnecting\n", state->name  );
        //    pto->fDisconnect = true;
        //}


    LogPrintf("*** RGP SendMessages Debug 0013 \n");
        ...
        //CNodeState *
       //state = State( pto->GetId() );   // State(nodeid); RGP
