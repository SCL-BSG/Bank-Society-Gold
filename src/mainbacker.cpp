...
// Copyright (c) 2019 Bank SocietyG Coin Developers
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    */
    ...
    /* ----------------------------------------------------------------
       -- RGP, Check for MAXIMUM coin height, which is approximate   --
       -- due to PoS rewards. KIll all rewards past block 75 000 000 --
       ---------------------------------------------------------------- */
    if ( nHeight > 75000000 )
    {
        LogPrintf("MAXIMUM Block height Exceeded!!!\n");
        return 0;
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    /* ----------------------------------------------------------------
       ...
       -- due to PoS rewards. KIll all rewards past block 75 000 000 --
       ---------------------------------------------------------------- */
    if ( pindexPrev->nHeight > 75000000 )
    {
        LogPrintf("MAXIMUM Block height Exceeded!!!\n");
        return 0;
    }
    ...
    LogPrintf("Coin Stake creation GetProofOfStakeReward(): create=%s nCoinAge=%d\n", FormatMoney(nSubsidy), nCoinAge);
...
bool PoS_Mining_Block;
    ...
    if (mapOrphanBlocks.count(hash))
    {
        ...
        if (!pblock->AcceptBlock())
        {
            ...
            if ( BSC_Wallet_Synching )
            {
                PushGetBlocks(pfrom, pindexBest, 0 ); /* also ask for everything from current block height */
        ...
        {
            LogPrintf("*** RGP ProcessBlock, ACCEPT was SUCCESSFUL \n");
        ...
        if ( pblock->IsProofOfStake() )
        {
            if ( setStakeSeen.count(pblock->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash) )
            {
                ...
                else
                {
                   ...
                   {
                       /* RGP, there can be 6 seconds betwwen the current stored block and the
...
int64_t nSearchTime;
   ...
   static int64_t nLastCoinStakeSearchTime = GetAdjustedTime(); // startup timestamp
    ...
    LogPrintf("*** RGP Signblock, txCoinStake after %d \n",txCoinStake.nTime  );

    nSearchTime = txCoinStake.nTime; // search to current time

    LogPrintf("*** RGP Signblock, Searchtime %d stakesearchtime %d",nSearchTime , nLastCoinStakeSearchTime   );
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    LogPrintf("*** RGP ProcessGetData start \n");

    LOCK(cs_main);

    while (it != pfrom->vRecvGetData.end())
    {
         LogPrintf("*** RGP ProcessGetData debug 001 \n");

        // Don't bother if send buffer is too full to respond anyway
        if (pfrom->nSendSize >= SendBufferSize())
        {
            LogPrintf("*** RGP ProcessGetData debug 002 send size to big %d \n", pfrom->nSendSize );
        ...
        const CInv &inv = *it;
        {
            ...
            {
                 LogPrintf("*** RGP ProcessGetData debug 003  \n" );
            ...
            {
                LogPrintf("*** RGP ProcessGetData debug 004  \n" );
    ...
    LogPrintf("*** RGP ProcessGetData end \n");
...
static uint64_t message_ask_filter = 50;
...
CInv Problem_Blocks_Inv;
    ...
    {
        PushGetBlocks( pfrom,  pindexBest, 0);
    ...
    else if (strCommand == "inv")
    {
        ...
        //if ( fDebug )
        //{
            LogPrintf("*** RGP ProcessMessage inventory items %d \n", vInv.size());
        //}
        ...
        for ( nInventory_index = 0; nInventory_index <= vInv.size(); nInventory_index++)
        {
            ...
            LogPrintf("*** RGP ProcessMessage inventory items %d %s index %d \n", vInv.size(), Inventory_Item.hash.ToString()  , nInventory_index );


            if ( Inventory_Item.type != MSG_BLOCK )
            {

                LogPrintf("*** Process Inventory invalid MSG_TYPE!!! %d index %d from node %s \n", Inventory_Item.type, nInventory_index, pfrom->addr.ToString() );
            ...
            LogPrintf("net,  got inventory: %s  %s\n", Inventory_Item.ToString(), fAlreadyHave ? "have" : "new");

            if (!fAlreadyHave)
            {

                LogPrintf("*** RGP INV, Don't have inventory, it's new!! \n");

                /* RGP moved here as we should not presume that we have it by automaticall
                ...
                if (!fImporting)
                {
                   ...
                   LogPrintf("*** Asking for block as new %s \n", Inventory_Item.ToString() );
                   ...
                   {
                       LogPrintf("*** Asking for block AGAIN %s \n", Inventory_Item.ToString() );
                ...
                LogPrintf("*** Asking Next blocks from PindexBest %s \n", pindexBest->GetBlockHash().ToString() );
            ...
            {
                LogPrintf("*** RGP Inventory Checking already have hash %s  \n", Inventory_Item.hash.ToString());
        ...
        // Track requests for our stuff
        //g_signals.Inventory( vInv.hash );

        MilliSleep( 1 );

        LogPrintf("*** RGP End of INV processing \n");
    }
    ...
    else if (strCommand == "getblocks")
    {
        ...
        int block_count_to_send;            /* RGP may need a bigger type such as long long?  */
        int limit_to_send;
        int maximum_blocks;
        uint256 xxx;
        ...
        LogPrintf("*** RGP ProcessMessage FIRST blocks items %s  from  %s \n", pindex->GetBlockHash().ToString() , pfrom->addr.ToStringIP() );


        if ( BSC_Wallet_Synching )
        {

            /* RGP Establish our current blockchain height to see how many blocks we can send */
            LogPrintf("*** RGP ProcessMessage getblocks, BSC_WALLET_SYNCHING our blockchain height is %d \n", pindex->nHeight );



            CBlockIndex* pindex_to_be_sent;

            LogPrintf("*** RGP ProcessMessage FIRST blocks hashstop %s from  %s \n", hashStop.ToString() , pfrom->addr.ToStringIP());

            /* RGP, now try to locate the exact blockchain position from the requester */
            std::map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.find(hashStop);
            if (mi != mapBlockIndex.end())
            {

                LogPrintf("*** RGP ProcessMessage FIRST blocks Found the hashstop %s \n", hashStop.ToString() );


                CBlockIndex* pindex_to_be_sent = (*mi).second;
                if (pindex_to_be_sent->IsInMainChain())
                {

                    /* RGP, Establish the hashstop height against our blockchain height, can we send anything? */


                    if ( pindex->nHeight - pindex_to_be_sent->nHeight )
                    {
                        /* send up to 500 entries to Limit */
                        limit_to_send = pindex->nHeight - pindex_to_be_sent->nHeight;
                        block_count_to_send = 0;
                        do
                        {
                            if ( pindex->pnext != nullptr )
                            {
                                pindex = pindex->pnext;    /* get next blockchain entry */

                                LogPrintf("*** RGP ProcessMessage FIRST blocks PUSHING %s to  %s \n", pindex->GetBlockHash().ToString() , pfrom->addr.ToStringIP());
                            ...
                            {
                                /* Reached the end of the blockchain valid records */
                                break;
                            }


                        } while ( block_count_to_send < 20 );

                        /* End of block sequence, finish up  */
                        pfrom->hashContinue = pindex->GetBlockHash(); /* RGP */

                    }
                    else
                    {
                        /* Send the same block back */
                        pfrom->PushInventory(  CInv(MSG_BLOCK, pindex->GetBlockHash() ) );
                        pfrom->hashContinue = pindex->GetBlockHash(); /* RGP */
                    }
                ...
                {
                    LogPrintf("*** RGP HashSTOP is ZERO \n");

                    /* Send the same block back */
                    pfrom->PushInventory(  CInv(MSG_BLOCK, pindex->GetBlockHash() ) );
                    pindex = pindex->pprev;    /* get next blockchain entry */
                    pfrom->PushInventory(  CInv(MSG_BLOCK, pindex->GetBlockHash() ) );
                    pindex = pindex->pprev;    /* get next blockchain entry */
                    pfrom->PushInventory(  CInv(MSG_BLOCK, pindex->GetBlockHash() ) );
                    pfrom->hashContinue = pindex->GetBlockHash(); /* RGP */
            ...
            else
            {
                   ...
                   LogPrintf("*** RGP ProcessMessage FIRST blocks HASHTOP was ZERO %s to  %s \n", pindex->GetBlockHash().ToString() , pfrom->addr.ToStringIP());
                   ...
                   do
                   {
                       ...
                       {
                           LogPrintf("*** RGP loopback BREAK %d \n ", pindex->nHeight );
                           break;
                       }
                   ...
                   LogPrintf("*** RGP loopback to %d \n ", pindex->nHeight );

                   LogPrintf("*** RGP GETBLOCKHASH %s \n", pindex->GetBlockHash().ToString());

                   hashStop = pindex->GetBlockHash();
                   CBlockIndex* pindex_to_be_sent;
                   ...
                   if (mi != mapBlockIndex.end())
                   {
                       ...
                       if (pindex_to_be_sent->IsInMainChain())
                       {
                           ...
                           do
                           {
                               LogPrintf("*** RGP sending block %d hash %s \n", maximum_blocks, pindex_to_be_sent->GetBlockHash().ToString()  );
                               pfrom->PushInventory(  CInv(MSG_BLOCK, pindex_to_be_sent->GetBlockHash() ) );
                               if ( pindex_to_be_sent->pnext == NULL )
                               {
                                   LogPrintf("*** RGP ppnext is NULL \n " );

                                   break;
                               }
                               else
                               {
                                   pindex_to_be_sent = pindex_to_be_sent->pnext;    /* get next blockchain entry */
                                   maximum_blocks++;
                               }

                           } while ( maximum_blocks < 100 );

                            /* Send the same block back */
                            //pfrom->PushInventory(  CInv(MSG_BLOCK, pindex_to_be_sent->GetBlockHash() ) );
                            //pindex_to_be_sent = pindex_to_be_sent->pnext;    /* get next blockchain entry */

                            //LogPrintf("*** RGP ProcessMessage FIRST blocks HASHSTOP was ZERO %s to  %s \n", pindex_to_be_sent->GetBlockHash().ToString() , pfrom->addr.ToStringIP());
                            ...
                            pfrom->hashContinue = pindex_to_be_sent->GetBlockHash(); /* RGP */
                       }
        ...
        else
        {
            ...
            LogPrintf("*** RGP ProcessMessage getblocks, NOT SYNCHING \n");
            ...
            for (; pindex; pindex = pindex->pnext)
            {
                ...
                if (pindex->GetBlockHash() == hashStop)
                {
                    ...
                    LogPrintf("*** RGP getblocks stopping at height %d hash %s \n", pindex->nHeight,  pindex->GetBlockHash().ToString() );
                    pfrom->hashContinue = pindex->GetBlockHash(); /* RGP */
                    break;
                }
                pfrom->PushInventory(CInv(MSG_BLOCK, pindex->GetBlockHash()));
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrintf("*** RGP ProcessMessage block message, start processing %s \n", hashBlock.ToString() );
        ...
        else
        {
            ...
            LogPrintf("*** RGP debug Process Block Debug 001 \n");


            /* RGP, Software Problem Domain, AcceptBlock() returns true on error, which
                    is a crazy scenario. Therefore, some additional checks need to be made
                    here and Orphans updated, where 'prev block not found' errors after AcceptBlock() */

            /* Search for the has in mapblockindex */
            /* If not found add Orphans for other checks */

            if ( !mapBlockIndex.count( block.GetHash() ) )
            {
                    /* Not in mapBlockIndex, as it was rejected */
                    LogPrintf("*** RGP After ProcessBlock check mapBlockIndex, not seen, must have been rejected by AcceptBlock \n");

                    /* Check if it's in Orphans map */
                    //if ( !mapOrphanBlocks.count( block.GetHash() ) )
                    //{
                        /* Accept Block failed, add this to Orphans */

                    //    LogPrintf("*** RGP check mapOrphanBlocks, not seen, Add Orphan Block! \n");
                    ...
                    //        pblock2->vchBlock = std::vector<unsigned char>(ss.begin(), ss.end());
                    //    }
                    //    pblock2->hashBlock = block.GetHash();
                    //    pblock2->hashPrev = block.hashPrevBlock;
                    //    pblock2->stake = block.GetProofOfStake();

                    //    LogPrintf("*** RGP debug Process Block Debug 081 \n");

                    //    mapOrphanBlocks.insert(make_pair( block.GetHash(), pblock2));
                    //    mapOrphanBlocksByPrev.insert(make_pair(pblock2->hashPrev, pblock2));

                    //    LogPrintf("*** RGP debug Process Block Debug 091 \n");

                        /* RGP test to try to get more accurate blocks */
                   //     PushGetBlocks( pfrom,  pindexBest, block.GetHash() );
                   //     PushGetBlocks( pfrom,  pindexBest, pblock2->hashPrev );

                   // }

                    /* Test that Orphans may have the next block in sequence, this will
                       will be performed until blocks in sequence are found               */

                    LogPrintf("*** RGP After ProcessBlock check Orphans to be done yet!! \n");

                    if ( mapOrphanBlocksByPrev.count( pindexBest->phashBlock ) )
                    {
                        LogPrintf("*** RGP FOUND pindex best in mapOrphanBlocksByPrev!! \n");
                    ...
                    else
                    {
                        ...
                        Inventory_1.type = MSG_BLOCK;
                        Inventory_1.hash = pindexBest->phashBlock;
                        pfrom->AskFor( Inventory_1, false   );
            ...
            LogPrintf("*** RGP debug Process Block Debug 999 \n");

            //int lost_block_check;
            //for( lost_block_check = 0; lost_block_check < mapOrphanBlocks.size(); lost_block_check++ )
            //{
...
string strCommand;
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end())
    {
        ...
        if (pfrom->nSendSize >= SendBufferSize())
        {
            LogPrintf("*** RGP Sendbuffer to fullfrom %s \n", pfrom->addr.ToString() );
            break;
        }
 ...
 //           MilliSleep( 1 ); /* RGP Optimize */
  ...
  //      }
        ...
        if ( !IsInitialBlockDownload() && ( GetTime() - nLastRebroadcast > 24 * 60 * 60 ) )
        {
            ...
            //LogPrintf("*** RGP SendMessage Refresh Broadcast\n");
        ...
        {
            LogPrintf("*** RGP SENDMESSAGE, Inventory \n");
        }
        ...
        vector<CInv> vInvWait;
        {
            ...
            {
                LogPrintf("*** RGP Send to node %s type %d hash %s \n", pto->addr.ToString(), inv.type, inv.hash.ToString()   );

                if ( pto->setInventoryKnown.count(inv) )
                {
                    LogPrintf("*** RGP Send to node inventory known %s \n", inv.hash.ToString() );
                    LogPrintf("*** RGP ProcessMessage inventory items %d hash %s \n", vInv.size(), inv.hash.ToString() );
                ...
                {
                    LogPrintf("*** RGP Send to debug 001 node %s type %d is MSG_TX \n", pto->addr.ToString(), inv.type);
                    ...
                    if (fTrickleWait)
                    {
                        LogPrintf("*** RGP Send to debug 002 node %s type %d is MSG_TX Hash is %s \n", pto->addr.ToString(), inv.type, inv.hash.ToString() );
                ...
                {
                    LogPrintf("*** RGP Send to debug 003 node %s type %d is MSG_TX Hash is %s \n", pto->addr.ToString(), inv.type, inv.hash.ToString() );


                    vInv.push_back(inv);
                    if (vInv.size() >= 1000)
                    {
                        LogPrintf("*** RGP Send to debug 004 node %s type %d is MSG_TX Hash is %s \n", pto->addr.ToString(), inv.type, inv.hash.ToString() );

                        pto->PushMessage("inv", vInv);
                        vInv.clear();
                    }
                    else
                        LogPrintf("*** RGP Send to debug 005 current inventory size to push %d ", vInv.size() );
                }
        ...
        if (!vInv.empty())
        {
            LogPrintf("*** RGP Send to debug 005 node %s  \n", pto->addr.ToString());
        ...
        while (!pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow)
        {
            ...
            LogPrintf("*** RGP SendMessages AskFor loop inventory %s \n", inv.ToString()  );
...
double test;
   ...
   // ret = blockValue * 3/4; //75%
    ...
    test = 0.0;
    test = (double)  blockValue;
    test = test * 4/10;
    blockValue = ( int )test;
    //LogPrintf("RGP GetMasternodePayment %d %d \n ", test, blockValue );
    ret = blockValue;       // * 4/10; //40%

    //LogPrintf("RGP GetMasternodePayment blockvalue %d \n ", ret );
